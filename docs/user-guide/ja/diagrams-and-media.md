...
基本的な sequence 図では参加者、ライフライン、メッセージ、ノート、activation に対応します。
...
塗り付きのメッセージパス、未知の矢印、× 終端、エフェクト、未対応の sequence 装飾は、
...
塗りつぶしに置換せず、関係線単位の画像にフォールバックします。
未対応のマーカー形状や塗り、装飾付きラベル、複雑な sequence 構文、エフェクト、
未知の形状も画像にフォールバックします。
