...
| --- | --- | --- |
| Flowchart, class, state, requirement, block, and sequence | Node outlines, compartments, and lifelines use the palette's muted text color rather than its faint decorative border. Group boundaries use the accent color. | Source-authored styles still take precedence. Dense diagrams can need shorter labels or fewer elements to remain readable at slide size. |
| ER | Entity headers and alternating attribute rows use the surface/code backgrounds with foreground text. The bundled renderer's `rowOdd` and `rowEven` roles are set explicitly. | The older `attributeBackgroundColorOdd/Even` variables do not control the unified ER renderer's rows. |
| Packet and tree view | Packet fields, bit numbers, title, tree labels, and hierarchy lines use their dedicated nested theme roles. | Custom themes need readable surface/foreground and background/muted pairs. |
| Mindmap, timeline, journey, kanban, and treemap | Distinct category hues are derived from the theme accent with luminance adjusted for foreground text and visible fills. Scale labels, peers, inverse colors, root variants, and Journey fills are mapped explicitly. Dark mode follows the resolved background/foreground pair. | Treemap leaf-label indexes differ from parent-fill indexes, so all categories use the same readable foreground rather than independently switching between black and white. A low-contrast or translucent custom palette cannot guarantee both text and shape contrast. |
| XY and Gantt | XY bars use a visible category color and the first line uses foreground. Gantt maps normal, active, completed, and critical task fills and text; critical tasks retain a distinct red tint/border. | Explicit plot styles remain source-controlled. Fine labels in dense charts still require enough display space. |
