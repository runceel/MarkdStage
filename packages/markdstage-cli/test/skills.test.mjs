...
test("all generated skills include the shipping Adaptive Cards contract", async () => {
  const guide = (await readGuide("adaptive-cards")).trim();
  for (const target of Object.keys(SKILL_TARGETS)) {
    const files = await buildSkillFiles(target);
    assert.ok(files.get("references/adaptive-cards.md").includes(guide));
    assert.match(files.get("SKILL.md"), /references\/adaptive-cards\.md/);
    assert.match(files.get("SKILL.md"), /not editable card objects/);
    assert.match(files.get("references/adaptive-cards.md"), /resourceValidation: "deferred-to-browser"/);
  }
});
