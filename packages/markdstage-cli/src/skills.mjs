...
let defaultReadGuidePromise;

function loadDefaultReadGuide() {
  defaultReadGuidePromise ??= import("./runtime.mjs").then(({ readGuide }) => readGuide);
  return defaultReadGuidePromise;
}
 ...
 */
export async function buildSkillFiles(target, { readGuide } = {}) {
  if (!SKILL_TARGETS[target]) {
    throw new Error(`Unknown skill target: ${target}`);
  }
  const loadGuide = readGuide ?? await loadDefaultReadGuide();
  const files = new Map();
  files.set(
    "SKILL.md",
    `${frontMatter({
  ...
  for (const topic of GUIDE_TOPICS) {
    const content = (await loadGuide(topic)).trim();
    const parts = [
