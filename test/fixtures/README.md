...
| `mermaid/er-basic.mmd`, `mermaid/er-basic.svg` | Seeded `erDiagram` source with four attributed entities, alternating row backgrounds, all attribute columns and keys, Japanese/HTML line breaks, identifying/non-identifying relations, labels, and every Mermaid 11.15.0 crow's-foot cardinality at both ends plus fixed SVG generated with bundled Mermaid 11.15.0 (`theme: default`, `securityLevel: strict`, ID `fixture-er-basic`) |
| `mermaid/requirement-basic.mmd`, `mermaid/requirement-basic.svg` | Seeded `requirementDiagram` source with `direction LR`, styled and empty requirement/element compartments, every rendered requirement and element field, Japanese/HTML line breaks plus literal escaped whitespace, forward/reverse relation notation, and all seven relation labels with the bundled contains-start and arrow-end terminals; fixed SVG generated with bundled Mermaid 11.15.0 (`theme: default`, `securityLevel: strict`, ID `fixture-requirement-basic`) |
