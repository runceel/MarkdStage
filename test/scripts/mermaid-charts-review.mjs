// node test\scripts\mermaid-charts-review.mjs <absolute-output-directory>
// Then reuse render-kanban-block-review.ps1 (five diagram pages, source left / PowerPoint right).
...
const diagrams = [
  ["Quadrants: editable points and labels", "quadrant-basic"],
  ["XY: editable bars and line vertices", "xychart-basic"],
  ["XY: horizontal and negative range", "xychart-horizontal"],
  ["Quadrants: local point shadow", "quadrant-hybrid"],
  ["XY: local bar shadow and axis label", "xychart-hybrid"],
];
const fragments = [];
...
const markdown = ["# Quadrant and XY charts\n\nRank 13 editable / hybrid review", ...fragments].join("\n\n---\n\n");
await mkdir(output, { recursive: true });
await writeFile(join(output, "provenance.json"), JSON.stringify({
  commit: execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim(),
  changes: execFileSync("git", ["status", "--porcelain"], { encoding: "utf8" }).trim(),
...
for (const theme of ["dark", "light", "microsoft", "custom"]) {
  ...
  await withDeckServer({
    ...
    const counts = rendered.model.slides.slice(1, 6).map((slide) =>
    ...
    for (let slideIndex = 1; slideIndex < 6; slideIndex++) {
      ...
      paintOrder.push({
        ...
        ].sort((a, b) => a.zOrder - b.zOrder).map((object) => ({
          path: object.path, type: object.type, zOrder: object.zOrder, reason: object.reason,
        })),
      });
    ...
    console.log(`${theme}: ${join(directory, "editable-hybrid.pptx")} (chart fallback counts ${counts})`);
  });
}
