// node test\scripts\mermaid-ishikawa-review.mjs <absolute-output-directory>
// Then render-kanban-block-review.ps1 -ArtifactDirectory <directory> -PageNumbers (2..6).
...
const diagrams = [
  ["Ishikawa: native lines, labels, arrows / local head", "ishikawa-basic"],
  ["Ishikawa: measured multiline and nested causes", "ishikawa-multiline"],
  ["Ishikawa: local line, text and group effects", "ishikawa-hybrid"],
  ["Ishikawa: handDrawn local artwork / native labels", "ishikawa-handdrawn"],
  ["Ishikawa: all native (head hidden in source)", "ishikawa-native"],
];
const expectedFallbacks = [1, 1, 7, 22, 0];
const fragments = [];
for (const [title, name] of diagrams) {
  fragments.push(`## ${title}\n\n\`\`\`mermaid\n${await readFile(join(fixtureDirectory, `${name}.mmd`), "utf8")}\n\`\`\``);
}
const markdown = ["# Ishikawa editable export\n\nRank 16 native / hybrid review", ...fragments].join("\n\n---\n\n");
...
await writeFile(join(output, "provenance.json"), JSON.stringify({
  ...
  changes, bundledMermaid: "11.15.0", node: process.version, customPalette,
...
for (const theme of ["dark", "light", "microsoft", "custom"]) {
  ...
  await withDeckServer({
    ...
    console.log(`${theme}: ${join(directory, "editable-hybrid.pptx")} (Ishikawa fallback counts ${counts})`);
  });
}
