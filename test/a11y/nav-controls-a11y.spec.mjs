...
for (const theme of ["dark", "light", "microsoft"]) {
  test(`export notifications are accessible without stealing focus (${theme})`, async ({ page }) => {
    ...
    try {
      ...
      const exportPath = "D:\\Exports\\slides.pdf";
      await exportRoute.fulfill({ json: { ok: true, path: exportPath } });
      ...
      await expect(page.locator("#exportNotificationPath")).toHaveText(`Saved to: ${exportPath}`);
      const exportLink = page.getByRole("link", { name: exportPath });
      await expect(exportLink).toBeVisible();
      await expect(exportLink).toHaveAccessibleName(exportPath);
