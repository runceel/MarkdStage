...
export const ADAPTIVE_CARD_PROPERTY_CONTRACTS = [
  ...
  rule("size weight color isSubtle fontType", "Owned font sizes/weights/palette: only accent has a distinct role color; other roles use fg, subtle uses muted.", "Native equivalent font/paint, not guaranteed pixel-identical Office text."),
  rule("style", "TextBlock default/heading; container default/emphasis; Image default/Person; static input/action exceptions below.", "Native supported styling; Image Person is bounded raster. Not a host preset."),
