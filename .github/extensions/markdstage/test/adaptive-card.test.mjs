...
async function loadVendoredSdk() {
  const bytes = await reconstructAsset(
    resolve(extensionRoot, "vendor"), "adaptivecards.min.js", resolve(extensionRoot, "vendor/vendor-assets.lock.json"),
  );
  const denied = () => { throw new Error("HostConfig fingerprinting must not render, fetch or execute."); };
  ...
  return sandbox.AdaptiveCards;
}

test("semantic container styles and color roles resolve from the palette with backward-compatible fallback", async () => {
  const SDK = await loadVendoredSdk();
  const basePalette = {
    fontFamily: "Segoe UI, sans-serif", fg: "#f4f4f4", muted: "#a0a0a0",
    accent: "#7c5cff", surface: "#222222", border: "#444444",
  };
  ...
  const fallbackColors = fallbackConfig.containerStyles.default.foregroundColors;
  assert.equal(fallbackColors.accent.default, basePalette.accent);
  assert.equal(fallbackColors.good.default, basePalette.accent);
  assert.equal(fallbackColors.warning.default, basePalette.accent);
  assert.equal(fallbackColors.attention.default, basePalette.accent);

  // A theme that defines the new semantic tokens gets distinct roles and
  // surface/border tints for each new container style.
  const semanticPalette = {
    ...basePalette,
    primary: "#3355ff", success: "#1f9d55", info: "#0aa8c8", warning: "#d9a300", danger: "#d23f3f",
    surfaceSuccess: "#123322", surfaceInfo: "#0e2a33", surfaceWarning: "#332a0e", surfaceDanger: "#331515",
  ...
  assert.equal(semanticColors.warning.default, semanticPalette.warning);
  assert.equal(semanticColors.attention.default, semanticPalette.danger);
...
test("semantic container styles are accepted alongside default/emphasis", () => {
  for (const style of ["default", "emphasis", "success", "info", "warning", "danger"]) {
    assert.deepEqual(validateAdaptiveCardSource(JSON.stringify(card([{ type: "Container", items: [], style }]))).diagnostics, []);
  }
  rejects(card([{ type: "Container", items: [], style: "primary" }]), "invalid-property");
});
