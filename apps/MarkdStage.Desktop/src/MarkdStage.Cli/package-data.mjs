...
import { GUIDE_TOPICS } from "../../../../packages/markdstage-cli/src/guide-topics.mjs";

const root = fileURLToPath(new URL("../../../../", import.meta.url));
const guides = Object.fromEntries(await Promise.all(GUIDE_TOPICS.map(async topic => [topic, await readGuide(topic)])));
const skills = {};
for (const target of Object.keys(SKILL_TARGETS)) {
  skills[target] = Object.fromEntries(await buildSkillFiles(target, { readGuide }));
